...
1. `fetch_account_records.py` 接收 job 名、平台别名或 `--all`。
2. 脚本读取 `fetch_jobs.json`，把 `defaults` 和目标 job 配置合并，得到 `platform`、`data_type`、时间范围、分页参数、`node_ids` 和 `output`。
3. 命令行参数优先级高于配置文件，可临时覆盖时间、分页、`node_id`、输出路径等字段。
4. `account_record_fetcher.py` 通过 `load_settings()` 读取 `.env`，拿到 `A6M1N_TOKEN` 和 `A6M1N_BASE_URL`。
5. 请求头使用 `Authorization: Bearer <token>` 和 `Platform: <platform>`，请求体由 `build_check_all_user_payload()` 生成。
6. 底层接口固定为 `/api/accountRecordCheckResult/checkAllUser`。
7. 默认调用 `fetch_check_all_users()` 循环抓取所有分页；加 `--single-page` 时只调用 `fetch_check_all_user()` 抓指定页。
8. 如果 job 配了多个 `node_ids`，会逐个 node 拉取，并把各响应的 `data.content` 合并；合并后的用户分组带 `nodeId`。
9. 最后 `write_output()` 把完整 JSON 写到 job 配置的 `output`，即 `inputs/<platform>/..._check_result.json`。

`account_record_workflow.py` 是“拉取并分析”的组合入口：先复用 `account_record_fetcher.py` 抓原始 JSON，再交给 `bill_rule_analyzer.py` 生成报告；需要保存原始 JSON 时传 `--raw-output inputs/...json`。
...
当前配置中，拼多多 job 会抓 1-2 库，抖店 job 会抓 1-3 库，淘宝 job 会抓 1-10 库，其他平台默认只抓 1 库。只抓库三：
